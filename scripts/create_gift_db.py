...
def analyze_database(gifts: List[Dict], logger) -> Dict:
    ...
    try:
        ...
        for gift in gifts:
            ...
            stats['sentiment_ranges'][range_key] = stats['sentiment_ranges'].get(range_key, 0) + 1
...
def main():
    parser = argparse.ArgumentParser(
        description="Create and manage gift database"
    )
    parser.add_argument(
        '--mode',
    ...
    parser.add_argument(
        '--output', '-o',
        help='Output file path for new database'
    )
    parser.add_argument(
        '--verbose', '-v',
        action='store_true',
